...
bool isSafe(const std::vector<std::string>& board, int row, int col) {
    for (size_t i = 0; i < static_cast<size_t>(row); ++i) {
        ...
        if (col + static_cast<int>(row) - static_cast<int>(i) < static_cast<int>(board.size()) && board[i][col + static_cast<int>(row) - static_cast<int>(i)] == '1') return false;
        if (col - (row - static_cast<int>(i)) >= 0 && board[i][col - (row - static_cast<int>(i))] == '1') return false;
        if (col + (row - static_cast<int>(i)) < static_cast<int>(board.size()) && board[i][col + (row - static_cast<int>(i))] == '1') return false;
    }
    return true;
}
